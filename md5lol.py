...
class md5lol():
	def __init__(self, p = None, i = stdin, o = stdout):
		...
		k = self._hkdf(32, p.encode('utf-8'))
		self.ek = k[0:16]
		self.ak = k[16:32]

	def _key_stream(self, size = 0):
		while size > len(self.km):
			self.km += self._key_stream_next()
		ret = self.km[:size]
		self.km = self.km[size:]
		return ret

	def _key_stream_next(self):
		bctr = binascii.a2b_hex("%016x" % self.ctr)
		if self.ctr == 0xffffffffffffffff:
			raise Exception('I dont gracefully handle ctr wraparound')
		self.ctr += 1
		return self._hmac(self.ek, bctr)

	def _hkdf(self, l = 32, ik = b''):
		dk = self._hmac(b'', ik)
		carry = b''
		out = b''
		x = 0
		while len(out) < l:
			carry = self._hmac(dk, carry + bytes([x+1]))
			x += 1
			out += carry
		return out[:l]

	def _hmac(self, k = b'', p = b''):
		return hmac.new(key = k, msg = p, digestmod = md5).digest()

	def _hmac_verify(self, m = b'', p = b'', k = b''):
		return hmac.compare_digest(m, hmac.new(key = k, msg = p, digestmod = md5).digest())

	def encrypt(self):
		p = self.i.read()
		k = self._key_stream(len(p))
		c = b''
		for a,b in zip(p,k):
			c += bytes([a ^ b])
		c = self._hmac(self.ak, c) + c
		self.o.write(c)

	def decrypt(self):
		c = self.i.read()
		if self._hmac_verify(m = c[:16], p = c[16:], k = self.ak) != True:
			raise Exception('Invalid message')
		c = c[16:]
		k = self._key_stream(len(c))
		p = b''
		for a,b in zip(c,k):
			p += bytes([a ^ b])
		self.o.write(p)

...
class Search():
    ...
    def match(self, test):
        ...
        for i in range(0,len(test)):
            if test[i] in tree:
                if tree[test[i]][1] is True and self.match(test[i+1:]) is True:
                    return True
                tree = tree[test[i]][0]
            else:
                return False
...
class Generator():
    ...
    def __init__(self, count = 500, wordlists = [], die = None, auto_start = False):
        self.die = die
        self.primes = []
        self.onions = {}
        self.count = count
        self.searcher = Search(wordlists)
        self.spinner = ['|','/','-','\\']
        if auto_start:
            while die.is_set():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--count", "-c", type = int, default = 100, help = "Number of primes to generate")
    parser.add_argument("--word-lists", "-w", type = str, default = "/usr/share/dict/cracklib-small", help = "Comma delimited list of wordlists")
    args = parser.parse_args()
    t = threading.Thread(target = Generator, args = (args.count, args.word_lists.split(','), die, True))
    t.start()
    try:
        while True:
            sleep(30.0)
